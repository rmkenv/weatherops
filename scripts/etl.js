/**
 * WeatherOps ETL — run by GitHub Actions every 15 minutes.
 * Writes JSON snapshots to /public/data/{locId}.json
 * so Next.js ISR and client-side cache fallback can serve stale data.
 */
const fs   = require("fs");
...
const HOURLY   = "temperature_2m,relative_humidity_2m,precipitation_probability,precipitation,weather_code,wind_speed_10m,uv_index";
...
async function main() {
  ...
  for (const loc of LOCATIONS) {
    try {
      ...
      const snapshot = { ts: Date.now(), weather, alerts };
      fs.writeFileSync(
        path.join(outDir, `${loc.id}.json`),
